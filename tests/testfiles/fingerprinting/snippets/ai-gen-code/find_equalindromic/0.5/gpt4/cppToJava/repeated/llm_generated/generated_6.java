...
public class Solution {
    ...
    public static void main(String[] args) {
        ...
        String line = in.nextLine(); // Read the input line
...
The Java code does essentially the same thing as the C++ code, but there are some differences due to the different languages. For example, Java does not have a native way to reverse a string, so the StringBuilder class is used instead. Similarly, Java does not have a native way to split a string into an array of integers, so the string is first split into an array of strings, and then each string is parsed into an integer.